...
@Transactional
@FileHandler
public class HisChDrugHandler extends AbstractHandler<DrugMapper> {
	...
	public List<DrugMapper> read(File file) throws Exception {
		...
		for(int i=0; i<excelList.size(); i++) {
			...
			String orgUsage = arr[2];		//数据库要加此字段
			...
			dm.setDosage_form("test");		//由于实体对剂型做了非空检查,所以在此处赋一个测试值
	...
	public void doProcess(List<DrugMapper> list) throws Exception {
		...
		if(StringUtil.isNotEmpty(currentOrgId)) {
			new Thread(new ZyypAutoMatchThread(drugMapperService, currentOrgId, importNo)).start();
		}
	}

	@Override
	public FileType getFileType() {
		return FileType.HIS_ZYYP;
	}
	
}

class ZyypAutoMatchThread implements Runnable {

	private String orgId;
	private String importNo;
	private DrugMapperService drugMapperService;
	
	public ZyypAutoMatchThread(DrugMapperService drugMapperService, String orgId, String importNo) {
	...
	@Override
	public void run() {
		drugMapperService.zyypAutoMatch(orgId, importNo);
	}
