...
@Transactional
@FileHandler
public class HisCpDrugHandler extends AbstractHandler<DrugMapper> {
	...
	public List<DrugMapper> read(File file) throws Exception {
		...
		for(int i=0; i<excelList.size(); i++) {
			...
			String orgDrugForm = StringUtil.emptyToNull(arr[2]);	//将空字符串转为null,否则此字段为空的无法自动对照
	...
	public void doProcess(List<DrugMapper> list) throws Exception {
		...
		System.out.println("invoke cpdrugHandler, importNo is " + importNo);
		String currentOrgId = "";
		for(DrugMapper dm : list) {
			...
			param.put("type", DrugType.ZCY.getType());
			if(drugMapperService.getCount(param) == 0L) {
				...
				dm.setType(DrugType.ZCY.getType());
		...
		if(StringUtil.isNotEmpty(currentOrgId)) {
			new Thread(new ZcyAutoMatchThread(drugMapperService, currentOrgId, importNo)).start();
		}
	}

	@Override
	public FileType getFileType() {
		return FileType.HIS_ZCY;
	}
}

class ZcyAutoMatchThread implements Runnable {

	private String orgId;
	private String importNo;
	private DrugMapperService drugMapperService;
	
	public ZcyAutoMatchThread(DrugMapperService drugMapperService, String orgId, String importNo) {
	...
	@Override
	public void run() {
		drugMapperService.zcyAutoMatch(orgId, importNo);
	}
