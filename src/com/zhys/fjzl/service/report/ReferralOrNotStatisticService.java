 ...
 *
 */
@Service("ReferralOrNotStatisticService")
@Transactional(isolation = Isolation.READ_COMMITTED, readOnly = true, rollbackFor = Throwable.class)
public class ReferralOrNotStatisticService extends BaseService {
	...
	public JSONObject getData(PageParam mapper, User user) throws Exception {
		...
		List<Map<Object, Object>> dataList = sqlDao.list("ReferralOrNotStatistic.main", mapper);
	...
	private JSONObject toJson(List<Map<Object, Object>> dataList) {
		...
		for(int i = 0; i < dataList.size(); i++) {
			...
			if(orgReferralMap.containsKey(orgName)) {
				Map<String, String> subMap = orgReferralMap.get(orgName);
				subMap.put(key, num);
			} else {
				Map<String, String> subMap = new HashMap<>();
				subMap.put(key, num);
				orgReferralMap.put(orgName, subMap);
			}
		}
		...
		json.put("orgNameArr", orgNameArr);
