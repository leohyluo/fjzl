...
<#assign list = statics["com.zhys.core.util.BeanUtil"].createInstance("java.util.ArrayList")>
<#list table.columnList as item>
    <#if item.ibatisType!="BLOB">
    ...
    <!--查询${table.annotation}-->
    <select id="query" parameterClass="${table.name}" resultClass="${table.name}">
        <![CDATA[
            select
            <#list list as field>
        ...
        <isParameterPresent prepend="where">
            <isEqual property="mode" compareValue="${table.pk}"><![CDATA[ ${table.pk}=#${table.pk}# ]]></isEqual>
        </isParameterPresent>
        <#--</dynamic>-->
