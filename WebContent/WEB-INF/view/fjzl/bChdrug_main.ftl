    ...
    <script type="text/javascript">
        $(function() {
            ...
            $("#table").datagrid({
                ...
                columns:[[
                    ...
                    {title:'药品名称',field:'name_',width:150,sortable:true},                   
                    {title:'用法用量',field:'reposit',width:150,sortable:true,hidden:true},
                    ...
                    {title:'药性',field:'phenotypictrait',width:150,sortable:true,hidden:true},
                    {title:'功效',field:'effect',width:150,sortable:true,hidden:true},
                    {title:'应用',field:'precautions',width:150,sortable:true,hidden:true},
                    {title:'注意事项',field:'standardcontrains',width:150,sortable:true,hidden:true},
                    {title:'化学成分',field:'chemicalcomposition',width:150,sortable:true,hidden:true},
                    {title:'药理作用',field:'pharmacologicalactions',width:150,sortable:true,hidden:true},
                    {title:'不良反应',field:'adversereaction',width:150,sortable:true,hidden:true},
                    ...
                    {title:'审核状态',field:'chdrug_status',width:150,sortable:true,formatter:function(value, data, index){
                    		if(value=="0"){
                    			return "未审核";
                    		}
                    		if(value=="1"){
                    ...
                    {title:'最后修改人',field:'last_updatorName',width:150,sortable:true,hidden:true},
            ...
            if(selections.length==0){
                ...
                $.messager.confirm("系统提示", "你确认要审核所选择的记录吗？", function(b){
                    if(b){
                        ...
                        $.ajaxPost("${path}/fjzl/bChdrug_reviewBatch.do", {"id_s":id_s}, function(result) {
                            $.messager.show({
                                title:"消息提醒",
                                msg:"审核成功",
                                timeout:5000,
            ...
            if(selections.length==0){
                ...
                $.messager.confirm("系统提示", "你确认要启用所选择的记录吗？", function(b){
                    if(b){
                        ...
                        $.ajaxPost("${path}/fjzl/bChdrug_enableYBatch.do", {"id_s":id_s}, function(result) {
                            $.messager.show({
                                title:"消息提醒",
                                msg:"记录启用成功",
                                timeout:5000,
		...
		function enableN(){
            ...
            if(selections.length==0){
            	...
            	$.ajaxPost("${path}/fjzl/bChdrug_beforeDisable.do", {"drug_status_in":"1,3,4,5","ids":ids,"type":"3"}, function(result) {
	            	if(result != "0") {
        ...
        function disableDrug(ids) {
        	$.ajaxPost("${path}/fjzl/bChdrug_enableNBatch.do", {"id_s":ids}, function(result) {
            	$.messager.show({
                	title:"消息提醒",
                    msg:"记录停用成功",
                    timeout:5000,
        ...
        function add() {
            ...
            $("#iframe").attr("src", "${path}/fjzl/bChdrug_add.do");
        }
        ...
        function edit(event, value) {
			...
			$.ajaxPost("${path}/fjzl/bChdrug_beforeEdit.do", {"drug_id":value,"drug_status_in":"1,3,4,5","type":"3"}, function(result) {
            	if(result != "0") {
            		var msg = "该数据已与机构建立对照关系,继续操作会导致机构已对照的数据需要重新对照,是否需要继续操作？";
            		$.messager.confirm('系统提示', msg, function(r){
            			if(r) {
		...
		function openEditDialog(value) {
			...
			$("#iframe").attr("src", "${path}/fjzl/bChdrug_edit.do?id_=" + value);
		}
        ...
        function del(event,val) {
            ...
            $.messager.confirm("提示信息", "你确认要删除该记录吗？", function(b) {
                if (b) {
                    $.ajaxPost("${path}/fjzl/bChdrug_delete.do",{"id_":val},function(result){
                        $.messager.show({
                            title:"消息提醒",
                            msg:"记录删除成功",
                            timeout:5000,
            ...
            if(selections.length==0){
                ...
                $.messager.confirm("系统提示", "你确认要删除所选择的记录吗？", function(b){
                    if(b){
                        ...
                        $.ajaxPost("${path}/fjzl/bChdrug_deleteBatch.do", {"id_s":id_s}, function(result) {
                            $.messager.show({
                                title:"消息提醒",
                                msg:"记录删除成功",
                                timeout:5000,
        ...
        function view(event,value){
            ...
            $("#iframe").attr("src","${path}/fjzl/bChdrug_view.do?id_="+value);
        }
        ...
        function refreshPage(){
            window.location="${path}/fjzl/bChdrug_main.do";
            return false;
        }
        ...
        function searchPage(reset) {
            if(reset) {
                ...
                $("#form select").val("");
                $("#form input:radio[name='chdrug_status']").attr('checked', false);
                $("#form input:radio[name='_enable']").attr('checked', false);
            }

            var bChdrug = $("#form").serializeJson();
            bChdrug.refresh = "1"; //刷新记录数参数
            $("#table").datagrid("clearSelections").datagrid("load",bChdrug);
        }
        /**
		*导入知识库中药饮片
		*/
		function upload() {
            ...
            $("#iframe").attr("src","${path}/fjzl/bChdrug_upload.do");
		}
		function download() {
		    window.location="${path}/fjzl/template_download.do?fileType=EBM_ZYYP";
			return false;
		}
    </script>
</head>
...
<div region="west" split="true" border="true" title="操作面板" style="width:250px;padding:10px;">
    <form id="form" name="form" style="display: none">
        <table class="table">
            <tr>
                <td class="th">药品名称</td>
                <td class="td"><input id="name_" name="name_" type="text" class="input"></td>
            </tr>
         <!--   <tr>
                <td class="th">药性</td>
                <td class="td"><input id="phenotypictrait" name="phenotypictrait" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">功效。作用</td>
                <td class="td"><input id="effect" name="effect" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">应用</td>
            ...
            <tr>
                <td class="th">用法用量</td>
                <td class="td"><input id="reposit" name="reposit" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">注意事项</td>
                <td class="td"><input id="standardcontrains" name="standardcontrains" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">化学成分</td>
                <td class="td"><input id="chemicalcomposition" name="chemicalcomposition" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">药理作用</td>
                <td class="td"><input id="pharmacologicalactions" name="pharmacologicalactions" type="text" class="input"></td>
            </tr>
            <tr>
                <td class="th">不良反应</td>
                <td class="td"><input id="adversereaction" name="adversereaction" type="text" class="input"></td>
            </tr>-->
            <tr>
                <td class="th">助记符</td>
            ...
            <td class="th">审核状态</td>
		        <td style="text-align:left">
		            <span class="radioSpan">
		                <input id="chdrug_status" name="chdrug_status" type="radio" value="0">未审核</input>
		                <input id="chdrug_status" name="chdrug_status" type="radio" value="1">已审核</input>
		            </span>
		        </td>
            </tr>
                    <tr>
...
<div region="center" title="当前位置：中药饮片管理">
    <table id="table"
           url="${path}/fjzl/bChdrug_page.do"
           border="false"         <#--无边框-->
           fit="true"             <#--自动填充宽度高度-->
