    ...
    <script type="text/javascript">
        $(function() {
            ...
            $("#table").datagrid({
                ...
                pageNumber:1,           
                pageSize:20,            
                queryParams:{"refresh":"1","type":"${type}","drug_status_in":"1,3,4,5",reviewFlag:"1"},
        ...
        var getColumns = function() {
			...
			if(drugType == 1 || drugType == 2) {
				data.push({title:'知识库药品名称',field:'ebmDrugName',width:150,sortable:true});
				data.push({title:'知识库药品剂型',field:'ebmDosageForm',width:150,sortable:true});
				data.push({title:'药品等级',field:'grade',width:150,sortable:true});
			} else if(drugType == 3) {
				data.push({title:'知识库药品名称',field:'ebmDrugName',width:150,sortable:true});
				data.push({title:'药品等级',field:'grade',width:150,sortable:true});
			}
			data.push({title:'规格',field:'specifications',width:150,sortable:true});
			data.push({title:'单位',field:'unit',width:150,sortable:true,hidden:true});
			data.push({title:'药物类型',field:'type',width:150,hidden:true,sortable:true,formatter:function(value, data, index){
                    		if(value=="1"){
                    			return "西药";
                    		}
                    		if(value=="2"){
            ...
            data.push({title:'状态',field:'drug_status',width:150,sortable:true,formatter:function(value, data, index){
                    		if(value=="0"){
                    			return "未对照";
                    		}
                    		if(value=="1"){
			...
			data.push({title:'是否启用',field:'_enable',width:150,sortable:true,formatter:function(value, data, index){
                    		if(value=="0"){
                    			return "已停用";
                    		}
                    		if(value=="1"){
			...
			data.push({title:'机构名称',field:'orgName',width:150,sortable:true});
			data.push({title:'创建时间',field:'create_time',width:150,sortable:true});
            data.push({title:'创建者',field:'creatorName',width:150,sortable:true});
            data.push({title:'最后修改时间',field:'last_update_time',width:150,sortable:true});
            data.push({title:'最后修改者',field:'last_updatorName',width:150,sortable:true});
            data.push({title:'审核时间',field:'review_time',width:150,sortable:true});
            data.push({title:'审核者',field:'revieworName',width:150,sortable:true});
            data.push({title:'备注',field:'memo',width:150,sortable:true});
        ...
        function add() {
            ...
            $("#iframe").attr("src", "${path}/fjzl/mapper_add.do?type=${type}");
        }
        ...
        function refreshPage(){
            window.location="${path}/fjzl/mapper_reviewMain.do?type=${type}";
            return false;
        }
		...
		*/
		function upload() {
			...
			var method_name = '';
			if(drugType == '1') {
				method_name = 'xyUpload';
			} else if (drugType == '2') {
				method_name = 'zcyUpload';
			} else if (drugType == '3') {
				method_name = 'zyypUpload';
			}
			$("#dialog").css("display","block").dialog({
                title: "导入数据",
                width:600,
            ...
            $("#iframe").attr("src","${path}/fjzl/mapper_"+method_name+".do");
		}
		function download() {
			var drugType = "${type}";
			var fileType = '';
		    ...
		    window.location="${path}/fjzl/template_download.do?fileType="+fileType;
			return false;
		}
    </script>
</head>
           ...
           <!-- <tr>
            ...
            <tr>
              ...
              <tr>
                <td class="th">状态</td>
                <td class="td">
                 <select id="drug_status" class="easyui-combobox" name="drug_status" panelMaxHeight="200" style="width:130px;">
...
<#if type==1> 
	<div region="center" title="当前位置：机构已对照西药管理">
</#if>
<#if type==2>
	<div region="center" title="当前位置：机构已对照中成药管理">
</#if>
<#if type==3>
	<div region="center" title="当前位置：机构已对照中药饮片管理">
</#if>
    <table id="table"
           url="${path}/fjzl/mapper_page.do?type=${type}"
           border="false"         <#--无边框-->
           fit="true"             <#--自动填充宽度高度-->
