 ...
 /**
		...
		function EnableN(){
            ...
            if(selections.length==0){
             ...
             $.ajaxPost("${path}/fjzl/bodyCheck_beforeDisable.do", {"ids":ids}, function(result) {
	            	if(result != "0") {
        ...
        function edit(event, value) {
			...
			$.ajaxPost("${path}/fjzl/bodyCheck_beforeEdit.do", {"check_id":value,"body_check_status_in":"1,3,4,5"}, function(result) {
            	if(result != "0") {
            		var msg = "该数据已与机构建立对照关系,继续操作会导致机构已对照的数据需要重新对照,是否需要继续操作？";
            		$.messager.confirm('系统提示', msg, function(r){
            			if(r) {
        ...
        function refreshPage(){
            window.location="${path}/fjzl/bodyCheck_main.do";
            return false;
        }
        ...
        function searchPage(reset) {
            if(reset) {
                ...
                $("#form select").val("");
               $("#form input:radio[name='body_check_status']").attr('checked', false);
...
<div region="center" title="当前位置：知识库检查管理">
    <table id="table"
